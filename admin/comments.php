...
if (!isset($session_id) || $session_id != "$session" || $session_id == '') {
	...
	if (!isset($action)) {
		...
		echo'<h1>Fan comments</h1>';
		$get_match = mysqli_query($db_connect, "SELECT DATE_FORMAT(team_matches.MatchDateTime, '%b %D %Y at %H:%i') AS date,
			...
			WHERE MatchSeasonID = '$season_id' AND
			team_matches.MatchTypeID = team_match_types.MatchTypeID AND
			team_matches.MatchPlaceID = team_match_places.MatchPlaceID AND
			team_matches.MatchOpponent = team_opponents.OpponentID AND
			MatchID = '$match_id'
			LIMIT 1
		") or die(mysqli_error());
		while($data = mysqli_fetch_array($get_match)) {
			echo "<b>".$data['date'].", vs. ".$data['opponent']."<br>".$data['matchplace']."";
			if ($data['neutral'] == 1)
			echo "(neutral)";
			echo ": ".$data['type_name']."</b><br><br>\n";
		}
		...
		while($data = mysqli_fetch_array($query)) {
			...
			echo "</tr><tr>\n";
			echo "<td align='left' valign='top' colspan='2'><b>Comments:</b><br><textarea name='comments' cols='40' rows='10'>".$data['comm']."</textarea></td>\n";
			echo "</tr><tr>\n";
		...
		if (mysqli_num_rows($query) == 0) {
			echo "Nobody has made comment yet..";
		}
	...
	$get_matches = mysqli_query($db_connect, "SELECT
		...
		team_matches.MatchNeutral AS neutral,
		team_previews.PreviewText AS prew_text
		FROM (team_matches, team_match_types, team_match_places, team_opponents)
		LEFT OUTER JOIN team_previews ON team_matches.MatchID = team_previews.PreviewMatchID
		WHERE MatchSeasonID = '$season_id' AND
		team_matches.MatchTypeID = team_match_types.MatchTypeID AND
		team_matches.MatchPlaceID = team_match_places.MatchPlaceID AND
		team_matches.MatchOpponent = team_opponents.OpponentID
