...
if ($default_season_id != 0 && $default_match_type_id != 0) {
	...
	") or die(mysqli_error());
	$get_yellows = mysqli_query($db_connect, "SELECT
		P.PlayerID AS id, COUNT( Y.YellowCardPlayerID ) AS yellows
		FROM team_players P
	...
	") or die(mysqli_error());
	$get_reds = mysqli_query($db_connect, "SELECT
		P.PlayerID AS id, COUNT( R.RedCardPlayerID ) AS reds
		FROM team_players P
	...
	") or die(mysqli_error());
	$get_ins = mysqli_query($db_connect, "SELECT
		P.PlayerID AS id, COUNT( SU.SubstitutionPlayerIDIn ) AS ins
		FROM team_players P
...
while ($data = mysqli_fetch_array($get_players)) {
	...
	$temp = mysqli_query($db_connect, "SELECT
		...
		WHERE S.SubstitutionPlayerIDIn = '".$idt[$i]."'
	...
	$temp = mysqli_query($db_connect, "SELECT *
		...
		WHERE A.AppearancePlayerID = '".$idt[$i]."' AND
		A.AppearanceSeasonID LIKE '".$tdefault_season_id."' AND
		M.MatchID = A.AppearanceMatchID AND
		M.MatchTypeID LIKE '".$tdefault_match_type_id."'
	") or die(mysqli_error());
	while ($tdata = mysqli_fetch_array($temp)) {
	...
	$temp = mysqli_query($db_connect, "SELECT
		...
		M.MatchOvertime AS overtime
		FROM team_substitutions S, team_matches M
		WHERE S.SubstitutionPlayerIDOut = '".$idt[$i]."' AND
		S.SubstitutionSeasonID LIKE '".$tdefault_season_id."' AND
		M.MatchID = S.SubstitutionMatchID AND
		M.MatchTypeID LIKE '".$tdefault_match_type_id."'
	") or die(mysqli_error());
	while ($tdata = mysqli_fetch_array($temp)) {
		if ($tdata['overtime'] == 0) {
	...
	$temp = mysqli_query($db_connect, "SELECT
		...
		WHERE R.RedCardPlayerID = '".$idt[$i]."' AND
		R.RedCardSeasonID LIKE '".$tdefault_season_id."' AND
		M.MatchID = R.RedCardMatchID AND
		M.MatchTypeID LIKE '".$tdefault_match_type_id."'
	") or die(mysqli_error());
	while ($tdata = mysqli_fetch_array($temp)) {
		if ($tdata['overtime'] == 0) {
...
if ($get_total > 0) {
	switch ($sort) {
		case 'name':
		array_multisort($names, SORT_ASC, SORT_STRING, $idt, $numbers, $goals, $apps, $yellows, $reds, $ins, $minutes, $publish);
		break;
		case 'number':
		array_multisort($numbers, SORT_ASC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $apps, $yellows, $reds, $ins, $minutes, $publish);
		break;
		case 'minutes':
		array_multisort($minutes, SORT_DESC, SORT_NUMERIC, $apps, SORT_DESC, SORT_NUMERIC, $ins, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $numbers, $yellows, $reds, $minutes, $publish);
		break;
		case 'apps':
		array_multisort($apps, SORT_DESC, SORT_NUMERIC, $ins, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $numbers, $yellows, $reds, $minutes, $publish);
		break;
		case 'ins':
		array_multisort($ins, SORT_DESC, SORT_NUMERIC, $apps, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $numbers, $yellows, $reds, $minutes, $publish);
		break;
		case 'goals':
		array_multisort($goals, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $numbers, $apps, $yellows, $reds, $ins, $minutes, $publish);
		break;
		case 'yellows':
		array_multisort($yellows, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $apps, $numbers, $reds, $ins, $minutes, $publish);
		break;
		case 'reds':
		array_multisort($reds, SORT_DESC, SORT_NUMERIC, $names, SORT_ASC, SORT_STRING, $idt, $goals, $apps, $yellows, $numbers, $ins, $minutes, $publish);
		break;
	}
	$i = 0;
	$j = 1;
	while ($i < $get_total) {
		...
		echo "<tr>\n";
		echo "<td align='right' valign='middle' bgcolor='#".$temp."'>";
		switch ($sort) {
			case 'apps': {
				if ($i > 0) {
					if ($apps[$i] == $apps[$i-1]) {
		...
		if ($publish[$i] == 1) {
			echo "<a href='player.php?id=".$idt[$i]."'>".$names[$i]."</a>";
		} else {
			echo "".$names[$i]."";
		}
		echo "</td>\n";
		echo "<td align='center' valign='middle' bgcolor='#".$temp."'>";
		if ($sort == 'minutes')
			echo "<b>";
			echo "".$minutes[$i]."";
		if ($sort == 'minutes')
...
if (SHOW_STAFF == 1) {
	...
	$query = mysqli_query($db_connect, "SELECT
		M.ManagerID AS id,
		M.ManagerLastName AS lastname,
		M.ManagerFirstName AS firstname
		FROM team_managers M, team_matches MA, team_seasons_managers SM
		WHERE MA.MatchSeasonID LIKE '".$tdefault_season_id."' AND
		MA.MatchTypeID LIKE '".$tdefault_match_type_id."' AND
		M.ManagerID = SM.SeasonManagerID AND
		SM.SeasonID = MA.MatchSeasonID
		GROUP BY id
	...
	while ($data = mysqli_fetch_array($query)) {
		...
		$get_timeline = mysqli_query($db_connect, "SELECT
			...
			WHERE T.ManagerID = M.ManagerID AND
			M.ManagerID = '".$id."'
			ORDER BY start
		") or die(mysqli_error());
		...
		if ($y > 0) {
			$temp_to_query = ' AND (';
			$x = 1;
			while ($timedata = mysqli_fetch_array($get_timeline)) {
		...
		$get_matches = mysqli_query($db_connect, "SELECT
			...
			FROM team_matches M
			WHERE M.MatchSeasonID LIKE '".$tdefault_season_id."' AND
			M.MatchTypeID LIKE '".$tdefault_match_type_id."' ".$temp_to_query." AND
		...
		echo "<tr align='left' bgcolor='".$temp."'>\n";
		echo "<td align='left' valign='middle'>";
		if ($data['lastname'] == '') {
			echo "<a href='manager.php?id=".$data['id']."'>".$data['firstname']."</a>";
		} else {
			echo"<a href='manager.php?id=".$data['id']."'>".$data['lastname'].", ".$data['firstname']."</a>";
		}
		echo "</td>\n";
